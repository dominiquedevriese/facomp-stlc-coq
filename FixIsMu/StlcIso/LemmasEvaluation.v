...
Section InvertECtxEq.
  ...
  Lemma ectxAppEqExtendPFold {t C t' C'} :
    EctxAppEq t C t' C' →
    EctxAppEq t (pfold C) t' (pfold C').
  Proof.
    eapply (ectxAppEqExtend (pfold phole)).
  Qed.

  Lemma ectxAppEqExtendPUnfold {t C t' C'} :
    EctxAppEq t C t' C' →
    EctxAppEq t (punfold C) t' (punfold C').
  Proof.
    eapply (ectxAppEqExtend (punfold phole)).
  Qed.
  ...
  Proof.
    ...
    (destruct C';
      ...
      |idtac..]);
    crush;
    eauto using ectxAppEqExtendPAbs, (* ectxAppEqExtendPProj₂, *) (* ectxAppEqExtendPProj₁, *) ectxAppEqExtendPInr, ectxAppEqExtendPInl, ectxAppEqExtendPApp₁, ectxAppEqExtendPApp₂, (* ectxAppEqExtendPPair₁, *) (* ectxAppEqExtendPPair₂, *) (* ectxAppEqExtendPIte₁, *) ectxAppEqExtendPCaseof₁, ectxAppEqExtendPFold, ectxAppEqExtendPUnfold (* ectxAppEqExtendPSeq₁, *) (* ectxAppEqExtendPFixt *);
...
Ltac crushImpossibleEvals :=
  match goal with
        ...
        | [ Hx : Value ?x,  H' : fold_ ?x --> _ |- _ ] => exfalso; refine (values_are_normal' _ _ H'); crush
  end.
...
Ltac inferContext :=
  ...
  let rec inferC acc t t₀ :=
      match t with
        ...
        (* | proj₂ ?t1 => inferC (pctx_cat (pproj₂ phole) acc) t1 t₀ *)
        | fold_ ?t1 => inferC (pctx_cat (pfold phole) acc) t1 t₀
        | unfold_ ?t1 => inferC (pctx_cat (punfold phole) acc) t1 t₀
...
Proof.
  assert (pctx_app (fold_ (inl t)) C' = pctx_app t (pctx_cat (pfold (pinl phole)) C')).
  inferContext.
  trivial.
Qed.
