...
Inductive Tm : Set :=
  ...
  | inr (t: Tm)
  | caseof (t₁ t₂ t₃: Tm)
  | fold_ (t : Tm) (* may need type parameters here *)
  | unfold_ (t : Tm).
  (* | seq (t₁ t₂: Tm) *)
  (* | fixt (τ₁ τ₂: Ty) (t: Tm). *)
...
Section WellScoping.
  ...
  Inductive wsTm (γ: Dom) : Tm → Prop :=
    ...
    | WsFold {t} : ⟨ γ ⊢ t ⟩ → ⟨ γ ⊢ fold_ t ⟩
    | WsUnfold {t} : ⟨ γ ⊢ t ⟩ → ⟨ γ ⊢ unfold_ t ⟩
  where "⟨ γ ⊢ t ⟩" := (wsTm γ t).
...
Fixpoint pctx_cat (C₀ C: PCtx) {struct C} : PCtx :=
  match C with
    ...
    | pfold C => pfold (pctx_cat C₀ C)
    | punfold C => punfold (pctx_cat C₀ C)
...
Section Application.
  ...
  Fixpoint apTm (ζ: Sub Y) (t: Tm) {struct t} : Tm :=
    match t with
      ...
      | fold_ t => fold_ (apTm ζ t)
      | unfold_ t => unfold_ (apTm ζ t)
    end.
...
Ltac crushStlcSyntaxMatchH :=
  match goal with
    ...
    | [ H: fold_ _        = fold_ _        |- _ ] => inversion H; clear H
    | [ H: unfold_ _        = unfold_ _        |- _ ] => inversion H; clear H
    ...
    | [ H: fold_ _       = abs _ _       |- _ ] => inversion H
    | [ H: unfold_ _       = abs _ _       |- _ ] => inversion H

    | [ |- S _          = S _          ] => f_equal
    | [ |- var _        = var _        ] => f_equal
    | [ |- abs _ _      = abs _ _      ] => f_equal
    | [ |- app _ _      = app _ _      ] => f_equal
    | [ |- fold_ _      = fold_ _      ] => f_equal
    | [ |- unfold_ _      = unfold_ _      ] => f_equal
