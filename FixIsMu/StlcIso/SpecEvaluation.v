...
Lemma eval_ind' (P: Tm → Tm → Prop) :
  ...
  (* (∀ t₁ t₂ : Tm, Value t₁ → Value t₂ → P (proj₂ (pair t₁ t₂)) t₂) → *)
  (∀ t : Tm, Value t → P (unfold_ (fold_ t)) t) →
  ...
  (∀ t t' : Tm, t --> t' → P t t' → P (fold_ t) (fold_ t')) →
  (∀ t t' : Tm, t --> t' → P t t' → P (unfold_ t) (unfold_ t')) →
  ...
  ∀ t t0 : Tm, t --> t0 → P t t0.
Proof.
  do 13 intro; induction 1 as [C ? ? r' ec].
  revert ec t t' r'.
